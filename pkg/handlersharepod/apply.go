...
func makeApplyHandler(defaultNamespace string, client clientset.Interface) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if miss == false && got != nil {
			...
			//need to change here for sharepod. (get and update function in client pkg in faas-netes)
...
//types.FunctionDeployment for now, need to fit for Sharepod, need new client pkg
...
func getResources(limits *types.FunctionResources) *sharepod.SharepodResources {
	if limits == nil {
		return nil
	}
	return &sharepod.SharepodResources{
		GPULimit: limits.CPU,
